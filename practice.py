...
def top_chars(phrase):
    ...
    # create initial dictionary to store statistics
    initial_dict = {}
    # remove spaces
    ...
    for phrase in phrases:
        ...
        for char in phrase:
            # count chars (might be done with dict[char] = phrase.count(char))
            initial_dict[char] = initial_dict.get(char, 0) + 1

    # invert dictionary to store statistics as keys and chars as values
    invert_dict = {}

    for k, v in initial_dict.items():
        invert_dict[v] = invert_dict.get(v, []) + [k]

    # sort inverted dictionary and return result
    res =  sorted(invert_dict.items())
